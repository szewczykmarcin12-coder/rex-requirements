...
import { Home, Folder, Bell, Settings, Search, ChevronLeft, FileText, ChevronRight, Menu, Lock, Plus, Trash2, Edit3, X, Upload, Save, LogOut, User, Shield, Eye, Download, File } from 'lucide-react';
...
const sectionTitles = {
  ...
  'lto': 'LTO',
  'delivery': 'DELIVERY',
  'wiedza-rex-sub': 'WIEDZA O REX CONCEPTS',
  'zarzadzanie-restauracja': 'ZARZĄDZANIE RESTAURACJĄ',
  'obszar-people': 'OBSZAR PEOPLE',
  'obszar-product': 'OBSZAR PRODUCT',
  'obszar-facility': 'OBSZAR FACILITY',
};
...
// Storage keys
const STORAGE_KEYS = {
  CATALOG: 'rex_catalog_data',
  FILES: 'rex_files_data',
};
...
function App() {
  ...
  const [filesData, setFilesData] = useState({}); // Przechowuje pliki PDF jako base64
  ...
  useEffect(() => {
    ...
    if (savedCatalog) {
      try {
        setCatalogData(JSON.parse(savedCatalog));
      } catch (e) {
    ...
    if (savedFiles) {
      try {
        setFilesData(JSON.parse(savedFiles));
      } catch (e) {
  ...
  // Save catalog data to localStorage whenever it changes
  useEffect(() => {
    if (catalogData !== initialCatalogData) {
      localStorage.setItem(STORAGE_KEYS.CATALOG, JSON.stringify(catalogData));
    }
  }, [catalogData]);

  // Save files data to localStorage whenever it changes
  useEffect(() => {
    if (Object.keys(filesData).length > 0) {
      localStorage.setItem(STORAGE_KEYS.FILES, JSON.stringify(filesData));
    }
  }, [filesData]);
  ...
  const handleFileSelect = (e) => {
    ...
    if (file && file.type === 'application/pdf') {
      ...
      if (!newItemName) {
        setNewItemName(file.name.replace('.pdf', ''));
      }
    } else {
  ...
  const openPdfViewer = (item) => {
    const pdfData = filesData[item.id];
    if (pdfData) {
      setCurrentPdf(pdfData);
      setCurrentPdfName(item.name);
      setShowPdfViewer(true);
    } else {
      alert('Plik PDF nie jest dostępny. To jest plik demonstracyjny bez zawartości.');
    }
  };
  ...
  const handleAddItem = async () => {
    ...
    // If it's a file and we have an uploaded file, convert to base64
    if (newItemType === 'file' && uploadedFile) {
      try {
        setUploadProgress(0);
  ...
  const handleDeleteItem = () => {
    ...
    // Remove file data if exists
    if (filesData[itemToDelete.id]) {
      setFilesData(prev => {
        const updated = { ...prev };
        delete updated[itemToDelete.id];
        return updated;
    ...
    return (
      <div className="w-full max-w-md mx-auto h-screen flex flex-col bg-rex-dark">
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-8">
          <div className="w-20 h-20 rounded-2xl flex items-center justify-center mb-4 overflow-hidden bg-white shadow-lg">
            <img src="/favicon.ico" alt="Rex Concepts" className="w-16 h-16 object-contain" />
          </div>
  ...
  if (showPdfViewer && currentPdf) {
    return (
      <div className="w-full max-w-md mx-auto h-screen flex flex-col bg-gray-900">
        {/* Header */}
        <header className="bg-rex-dark px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button 
              onClick={() => { setShowPdfViewer(false); setCurrentPdf(null); }}
      ...
      <header className="bg-rex-dark px-4 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          {currentPath.length > 1 ? (
            ...
            <img src="/favicon.ico" alt="Rex" className="w-8 h-8 object-contain" />
          )}
        ...
        {activeTab === 'home' && (
          <div className="space-y-3">
            {displayItems.map((item) => (
              ...
              >
                <button
                  onClick={() => item.type === 'folder' ? navigateToFolder(item.id) : openPdfViewer(item)}
                ...
                >
                  ...
                  <div className="flex-1 text-left min-w-0">
                    ...
                    <span className="text-gray-400 text-xs mt-0.5 block">
                      ...
                      {item.type === 'file' && filesData[item.id] && ' • Dostępny'}
                    </span>
                  </div>
                </button>
                ...
                {isAdmin ? (
                  <div className="flex items-center gap-1">
                    {item.type === 'file' && filesData[item.id] && (
                      <button
                        onClick={() => openPdfViewer(item)}
                        className="p-2 rounded-lg hover:bg-gray-100 transition-colors"
                      >
                        <Eye size={16} color="#395185" />
                      </button>
                    )}
                    <button
        ...
        {activeTab === 'settings' && (
          <div className="space-y-3">
            ...
            {isAdmin && (
              <button
                onClick={() => {
                  if (confirm('Czy na pewno chcesz zresetować wszystkie dane do ustawień początkowych?')) {
                ...
                className="w-full rounded-2xl p-4 font-medium text-sm shadow-sm transition-all hover:opacity-90 flex items-center justify-center gap-2 bg-gray-100 text-gray-600"
              >
                Resetuj dane do początkowych
              </button>
            )}
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                onClick={() => { setShowAddModal(false); setNewItemName(''); setUploadedFile(null); }}
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="flex gap-3">
                  <button
                    onClick={() => { setNewItemType('folder'); setUploadedFile(null); }}
              ...
              {newItemType === 'file' && (
                <div>
                  <label className="block text-gray-600 text-sm mb-1.5">Plik PDF</label>
                  <input
                    ref={fileInputRef}
                    type="file"
                  ...
                  {uploadedFile ? (
                    <div className="p-4 rounded-xl border-2 border-green-200 bg-green-50">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-gray-800 text-sm font-medium truncate">{uploadedFile.name}</p>
                    ...
                    <button
                      ...
                      className="w-full p-6 rounded-xl border-2 border-dashed border-gray-200 text-center hover:border-gray-300 hover:bg-gray-50 transition-all"
                    >
                      <Upload size={32} color="#9CA3AF" className="mx-auto mb-2" />
                      <p className="text-gray-500 text-sm">Kliknij, aby wybrać plik PDF</p>
                      <p className="text-gray-400 text-xs mt-1">Maksymalnie 10 MB</p>
                    </button>
                  )}
                </div>
              )}
              ...
              <button
                ...
                disabled={!newItemName.trim() || (newItemType === 'file' && !uploadedFile)}
